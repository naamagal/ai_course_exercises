...
class GraphPlan(object):
    ...
    def graph_plan(self, relax_mutex=False):
        """
        ...
        while self.goal_state_not_in_prop_layer(self.graph[level].get_proposition_layer().get_propositions()) or \
                (not relax_mutex and self.goal_state_has_mutex(self.graph[level].get_proposition_layer())):
            ...
            if relax_mutex:
                # calls the expand_without_mutex function, which implemented in the PlanGraph class
                pg_next.expand_without_mutex(self.graph[level - 1])
            else:
                # calls the expand function, which implemented in the PlanGraph class
        ...
        plan_solution = self.extract(self.graph, self.goal, level, relax_mutex)
    ...
    def extract(self, graph, sub_goals, level, relax_mutex=False):
        """
        ...
        if sub_goals in self.no_goods[level]:
            return None
        plan_solution = self.gp_search(graph, sub_goals, [], level, relax_mutex)
        if plan_solution is not None:
            return plan_solution
        self.no_goods[level].append([sub_goals])
        return None

    def gp_search(self, graph, sub_goals, _plan, level, relax_mutex=False):
        if len(sub_goals) == 0:
            new_goals = []
            for action in _plan:
            ...
            new_plan = self.extract(graph, new_goals, level - 1, relax_mutex)
            if new_plan is None:
                return None
            else:
        ...
        for action in providers:
            ...
            new_plan = self.gp_search(graph, new_sub_goals, plan_clone, level, relax_mutex)
