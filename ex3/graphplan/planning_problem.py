...
class PlanningProblem:
    def __init__(self, domain_file, problem_file):
        ...
        # frozenset: immutable and hashable -- its contents cannot be altered after is created
        ...
        # I added that
        self.graph_plan_helper = gp.GraphPlan(domain_file, problem_file)
        #self.last_from = -1
        #self.last_to = -1
    ...
    def is_goal_state(self, state):
        ...
        state is a frozenset of proposition (??)
        """
        "*** YOUR CODE HERE ***"
        return not self.goal_state_not_in_prop_layer(state)

    # def is_opposite_to_last_act(self, action):
    #     where_from = int(action[-3])
    #     where_to = int(action[-1])
    #     if where_from != self.last_to or where_to != self.last_from:
    #         self.last_from = where_from
    #         self.last_to = where_to
    #         return False
    #     self.last_from = where_from
    #     self.last_to = where_to
    #     return True
    ...
    def get_successors(self, state):
        ...
        For a set (or list) of propositions l and an action a,
        a.all_preconds_in_list(l) returns true if the preconditions of a are in l

        State is a list of propositions
        ...
        all_successors = []
        step_cost = 1
        # Go over all actions and check if they are available (i.e. preconditions are exists in the state)
        for action in self.actions:
            if action.all_preconds_in_list(state) and not action.is_noop(): #and not self.is_opposite_to_last_act(action.get_name()):
                # for each action create a triple
                triple_succ = (self.apply_action(state, action), action, step_cost)
                all_successors.append(triple_succ)
    ...
    def apply_action(self, state, action):
        next_state = []
        for prop in state:
            # Remove all from delete-list
            if not action.is_neg_effect(prop):
                next_state.append(prop)
        # Add all from add-list
        for new_prop in action.get_add():
            if not new_prop in next_state:
                next_state.append(new_prop)
...
def max_level(state, planning_problem):
    ...
    """
    "*** YOUR CODE HERE ***"
    planning_problem.graph_plan_helper.set_init_state(state)
    relaxed_mutex = True
    plan = planning_problem.graph_plan_helper.graph_plan(relaxed_mutex)
    if plan == None:
        return float('inf')
    plan_len = planning_problem.graph_plan_helper.get_plan_length()
    return plan_len-1
...
def level_sum(state, planning_problem):
    ...
    """
    "*** YOUR CODE HERE ***"
    gh = planning_problem.graph_plan_helper
    gh.set_init_state(state)
    relaxed_mutex = True
    plan = gh.graph_plan(relaxed_mutex)
    if plan is None:
        return float('inf')

    all_goals = [[goal, False] for goal in gh.goal]
    plan_len = gh.get_plan_length()
    sum_of_level = 0
    for level in range(0, plan_len):
        for g in all_goals:
            if not g[1] and g[0] in gh.graph[level].get_proposition_layer().get_propositions():
                g[1] = True
                sum_of_level += (level+1)
