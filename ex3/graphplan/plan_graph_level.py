...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for action in all_actions:
            precond = action.get_pre()
            isSatisfied = True
            if not previous_proposition_layer.all_preconds_in_layer(action):
                isSatisfied = False
            for cond in precond:
                for cond2 in precond:
                    if not cond == cond2:
                        if previous_proposition_layer.is_mutex(cond, cond2):
                            isSatisfied = False
                            break
    ...
    def update_mutex_actions(self, previous_layer_mutex_proposition):
        ...
        for action1 in current_layer_actions:
            for action2 in current_layer_actions:
                if action1 != action2 and not action1.is_noop() and not action2.is_noop() :
    ...
    def update_proposition_layer(self):
        ...
        for action in current_layer_actions:
            propsFromAction = action.get_add()
            for prop in propsFromAction:
                if prop.get_name in propDict:
                    propDict[prop.get_name].add_producer(action)
                else:
                    name = prop.get_name()
                    newProp = Proposition(name)
                    newProp.add_producer(action)
                    propDict[prop.get_name] = newProp
        for prop in propDict:
            self.proposition_layer.add_proposition(propDict[prop])
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    for a1PreCond in a1Pre:
        for a2PreCond in a2Pre:
            if Pair(a1PreCond, a2PreCond) in mutex_props or Pair(a2PreCond, a1PreCond) in mutex_props:
                return True
    return False
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    p1Procedures = prop1.get_producers()
    p2Procedures = prop2.get_producers()

    for action1 in p1Procedures:
        for action2 in p2Procedures:
            if not (Pair(action1, action2) in mutex_actions_list):
                return False
            # TODO - how do I check if they negate each other?
            # Why is it necessary here (Na'ama)?
    return True
